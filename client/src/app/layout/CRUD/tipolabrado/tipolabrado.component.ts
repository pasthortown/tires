...
@Component({
   selector: 'app-tipolabrado',
   templateUrl: './tipolabrado.component.html',
   styleUrls: ['./tipolabrado.component.scss']
})

export class TipoLabradoComponent implements OnInit {

   busy: Promise<any>;
   entidades: TipoLabrado[];
   entidadSeleccionada: TipoLabrado;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: TipoLabradoService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: TipoLabrado): boolean {
      return true;
   }
   ...
   crearEntidad(): TipoLabrado {
      const nuevoTipoLabrado = new TipoLabrado();
      nuevoTipoLabrado.id = 0;
      return nuevoTipoLabrado;
   }

   add(entidadNueva: TipoLabrado): void {
   ...
   update(entidadParaActualizar: TipoLabrado): void {
   ...
   delete(entidadParaBorrar: TipoLabrado): void {
   ...
   onSelect(entidadActual: TipoLabrado): void {
