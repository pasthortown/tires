...
@Component({
   selector: 'app-cabecerafacturaventa',
   templateUrl: './cabecerafacturaventa.component.html',
   styleUrls: ['./cabecerafacturaventa.component.scss']
})

export class CabeceraFacturaVentaComponent implements OnInit {

   busy: Promise<any>;
   entidades: CabeceraFacturaVenta[];
   entidadSeleccionada: CabeceraFacturaVenta;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: CabeceraFacturaVentaService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: CabeceraFacturaVenta): boolean {
      return true;
   }
   ...
   crearEntidad(): CabeceraFacturaVenta {
      const nuevoCabeceraFacturaVenta = new CabeceraFacturaVenta();
      nuevoCabeceraFacturaVenta.id = 0;
      return nuevoCabeceraFacturaVenta;
   }

   add(entidadNueva: CabeceraFacturaVenta): void {
   ...
   update(entidadParaActualizar: CabeceraFacturaVenta): void {
   ...
   delete(entidadParaBorrar: CabeceraFacturaVenta): void {
   ...
   refresh(): void {
      ...
      this.entidades = CabeceraFacturaVenta[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: CabeceraFacturaVenta): void {
