...
@Component({
   selector: 'app-indicecarga',
   templateUrl: './indicecarga.component.html',
   styleUrls: ['./indicecarga.component.scss']
})

export class IndiceCargaComponent implements OnInit {

   busy: Promise<any>;
   entidades: IndiceCarga[];
   entidadSeleccionada: IndiceCarga;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: IndiceCargaService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: IndiceCarga): boolean {
      return true;
   }
   ...
   crearEntidad(): IndiceCarga {
      const nuevoIndiceCarga = new IndiceCarga();
      nuevoIndiceCarga.id = 0;
      return nuevoIndiceCarga;
   }

   add(entidadNueva: IndiceCarga): void {
   ...
   update(entidadParaActualizar: IndiceCarga): void {
   ...
   delete(entidadParaBorrar: IndiceCarga): void {
   ...
   onSelect(entidadActual: IndiceCarga): void {
