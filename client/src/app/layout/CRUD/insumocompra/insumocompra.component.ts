...
@Component({
   selector: 'app-insumocompra',
   templateUrl: './insumocompra.component.html',
   styleUrls: ['./insumocompra.component.scss']
})

export class InsumoCompraComponent implements OnInit {

   busy: Promise<any>;
   entidades: InsumoCompra[];
   entidadSeleccionada: InsumoCompra;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: InsumoCompraService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: InsumoCompra): boolean {
      return true;
   }
   ...
   crearEntidad(): InsumoCompra {
      const nuevoInsumoCompra = new InsumoCompra();
      nuevoInsumoCompra.id = 0;
      return nuevoInsumoCompra;
   }

   add(entidadNueva: InsumoCompra): void {
   ...
   update(entidadParaActualizar: InsumoCompra): void {
   ...
   delete(entidadParaBorrar: InsumoCompra): void {
   ...
   onSelect(entidadActual: InsumoCompra): void {
