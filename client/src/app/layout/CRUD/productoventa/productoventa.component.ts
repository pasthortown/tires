...
@Component({
   selector: 'app-productoventa',
   templateUrl: './productoventa.component.html',
   styleUrls: ['./productoventa.component.scss']
})

export class ProductoVentaComponent implements OnInit {

   busy: Promise<any>;
   entidades: ProductoVenta[];
   entidadSeleccionada: ProductoVenta;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: ProductoVentaService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: ProductoVenta): boolean {
      return true;
   }
   ...
   crearEntidad(): ProductoVenta {
      const nuevoProductoVenta = new ProductoVenta();
      nuevoProductoVenta.id = 0;
      return nuevoProductoVenta;
   }

   add(entidadNueva: ProductoVenta): void {
   ...
   update(entidadParaActualizar: ProductoVenta): void {
   ...
   delete(entidadParaBorrar: ProductoVenta): void {
   ...
   refresh(): void {
      ...
      this.entidades = ProductoVenta[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: ProductoVenta): void {
