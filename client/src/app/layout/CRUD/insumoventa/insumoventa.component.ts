...
@Component({
   selector: 'app-insumoventa',
   templateUrl: './insumoventa.component.html',
   styleUrls: ['./insumoventa.component.scss']
})

export class InsumoVentaComponent implements OnInit {

   busy: Promise<any>;
   entidades: InsumoVenta[];
   entidadSeleccionada: InsumoVenta;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: InsumoVentaService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: InsumoVenta): boolean {
      return true;
   }
   ...
   crearEntidad(): InsumoVenta {
      const nuevoInsumoVenta = new InsumoVenta();
      nuevoInsumoVenta.id = 0;
      return nuevoInsumoVenta;
   }

   add(entidadNueva: InsumoVenta): void {
   ...
   update(entidadParaActualizar: InsumoVenta): void {
   ...
   delete(entidadParaBorrar: InsumoVenta): void {
   ...
   onSelect(entidadActual: InsumoVenta): void {
