...
export class TipoVehiculoComponent implements OnInit {
   ...
   busy: Promise<any>;
   entidades: TipoVehiculo[];
   entidadSeleccionada: TipoVehiculo;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: TipoVehiculoService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: TipoVehiculo): boolean {
      return true;
   }
   ...
   crearEntidad(): TipoVehiculo {
      const nuevoTipoVehiculo = new TipoVehiculo();
      nuevoTipoVehiculo.id = 0;
      return nuevoTipoVehiculo;
   }

   add(entidadNueva: TipoVehiculo): void {
   ...
   update(entidadParaActualizar: TipoVehiculo): void {
   ...
   delete(entidadParaBorrar: TipoVehiculo): void {
   ...
   onSelect(entidadActual: TipoVehiculo): void {
