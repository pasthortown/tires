...
@Component({
   selector: 'app-llantaventa',
   templateUrl: './llantaventa.component.html',
   styleUrls: ['./llantaventa.component.scss']
})

export class LlantaVentaComponent implements OnInit {

   busy: Promise<any>;
   entidades: LlantaVenta[];
   entidadSeleccionada: LlantaVenta;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: LlantaVentaService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: LlantaVenta): boolean {
      return true;
   }
   ...
   crearEntidad(): LlantaVenta {
      const nuevoLlantaVenta = new LlantaVenta();
      nuevoLlantaVenta.id = 0;
      return nuevoLlantaVenta;
   }

   add(entidadNueva: LlantaVenta): void {
   ...
   update(entidadParaActualizar: LlantaVenta): void {
   ...
   delete(entidadParaBorrar: LlantaVenta): void {
   ...
   onSelect(entidadActual: LlantaVenta): void {
