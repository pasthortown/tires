...
@Component({
   selector: 'app-caracteristicaterreno',
   templateUrl: './caracteristicaterreno.component.html',
   styleUrls: ['./caracteristicaterreno.component.scss']
})

export class CaracteristicaTerrenoComponent implements OnInit {

   busy: Promise<any>;
   entidades: CaracteristicaTerreno[];
   entidadSeleccionada: CaracteristicaTerreno;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: CaracteristicaTerrenoService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: CaracteristicaTerreno): boolean {
      return true;
   }
   ...
   crearEntidad(): CaracteristicaTerreno {
      const nuevoCaracteristicaTerreno = new CaracteristicaTerreno();
      nuevoCaracteristicaTerreno.id = 0;
      return nuevoCaracteristicaTerreno;
   }

   add(entidadNueva: CaracteristicaTerreno): void {
   ...
   update(entidadParaActualizar: CaracteristicaTerreno): void {
   ...
   delete(entidadParaBorrar: CaracteristicaTerreno): void {
   ...
   refresh(): void {
      ...
      this.entidades = CaracteristicaTerreno[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: CaracteristicaTerreno): void {
