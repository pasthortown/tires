...
@Component({
   selector: 'app-serviciocompra',
   templateUrl: './serviciocompra.component.html',
   styleUrls: ['./serviciocompra.component.scss']
})

export class ServicioCompraComponent implements OnInit {

   busy: Promise<any>;
   entidades: ServicioCompra[];
   entidadSeleccionada: ServicioCompra;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: ServicioCompraService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: ServicioCompra): boolean {
      return true;
   }
   ...
   crearEntidad(): ServicioCompra {
      const nuevoServicioCompra = new ServicioCompra();
      nuevoServicioCompra.id = 0;
      return nuevoServicioCompra;
   }

   add(entidadNueva: ServicioCompra): void {
   ...
   update(entidadParaActualizar: ServicioCompra): void {
   ...
   delete(entidadParaBorrar: ServicioCompra): void {
   ...
   refresh(): void {
      ...
      this.entidades = ServicioCompra[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: ServicioCompra): void {
