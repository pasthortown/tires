...
@Component({
   selector: 'app-motivofin',
   templateUrl: './motivofin.component.html',
   styleUrls: ['./motivofin.component.scss']
})

export class MotivoFinComponent implements OnInit {

   busy: Promise<any>;
   entidades: MotivoFin[];
   entidadSeleccionada: MotivoFin;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: MotivoFinService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: MotivoFin): boolean {
      return true;
   }
   ...
   crearEntidad(): MotivoFin {
      const nuevoMotivoFin = new MotivoFin();
      nuevoMotivoFin.id = 0;
      return nuevoMotivoFin;
   }

   add(entidadNueva: MotivoFin): void {
   ...
   update(entidadParaActualizar: MotivoFin): void {
   ...
   delete(entidadParaBorrar: MotivoFin): void {
   ...
   onSelect(entidadActual: MotivoFin): void {
