...
@Component({
   selector: 'app-servicioventa',
   templateUrl: './servicioventa.component.html',
   styleUrls: ['./servicioventa.component.scss']
})

export class ServicioVentaComponent implements OnInit {

   busy: Promise<any>;
   entidades: ServicioVenta[];
   entidadSeleccionada: ServicioVenta;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: ServicioVentaService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: ServicioVenta): boolean {
      return true;
   }
   ...
   crearEntidad(): ServicioVenta {
      const nuevoServicioVenta = new ServicioVenta();
      nuevoServicioVenta.id = 0;
      return nuevoServicioVenta;
   }

   add(entidadNueva: ServicioVenta): void {
   ...
   update(entidadParaActualizar: ServicioVenta): void {
   ...
   delete(entidadParaBorrar: ServicioVenta): void {
   ...
   refresh(): void {
      ...
      this.entidades = ServicioVenta[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: ServicioVenta): void {
