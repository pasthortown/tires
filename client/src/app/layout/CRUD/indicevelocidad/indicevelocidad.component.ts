...
@Component({
   selector: 'app-indicevelocidad',
   templateUrl: './indicevelocidad.component.html',
   styleUrls: ['./indicevelocidad.component.scss']
})

export class IndiceVelocidadComponent implements OnInit {

   busy: Promise<any>;
   entidades: IndiceVelocidad[];
   entidadSeleccionada: IndiceVelocidad;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: IndiceVelocidadService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: IndiceVelocidad): boolean {
      return true;
   }
   ...
   crearEntidad(): IndiceVelocidad {
      const nuevoIndiceVelocidad = new IndiceVelocidad();
      nuevoIndiceVelocidad.id = 0;
      return nuevoIndiceVelocidad;
   }

   add(entidadNueva: IndiceVelocidad): void {
   ...
   update(entidadParaActualizar: IndiceVelocidad): void {
   ...
   delete(entidadParaBorrar: IndiceVelocidad): void {
   ...
   refresh(): void {
      ...
      this.entidades = IndiceVelocidad[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: IndiceVelocidad): void {
