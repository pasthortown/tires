...
@Component({
   selector: 'app-baseiess',
   templateUrl: './baseiess.component.html',
   styleUrls: ['./baseiess.component.scss']
})

export class BaseIESSComponent implements OnInit {

   busy: Promise<any>;
   entidades: BaseIESS[];
   entidadSeleccionada: BaseIESS;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: BaseIESSService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: BaseIESS): boolean {
      return true;
   }
   ...
   crearEntidad(): BaseIESS {
      const nuevoBaseIESS = new BaseIESS();
      nuevoBaseIESS.id = 0;
      return nuevoBaseIESS;
   }

   add(entidadNueva: BaseIESS): void {
   ...
   update(entidadParaActualizar: BaseIESS): void {
   ...
   delete(entidadParaBorrar: BaseIESS): void {
   ...
   onSelect(entidadActual: BaseIESS): void {
