...
@Component({
   selector: 'app-cabecerafacturacompra',
   templateUrl: './cabecerafacturacompra.component.html',
   styleUrls: ['./cabecerafacturacompra.component.scss']
})

export class CabeceraFacturaCompraComponent implements OnInit {

   busy: Promise<any>;
   entidades: CabeceraFacturaCompra[];
   entidadSeleccionada: CabeceraFacturaCompra;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: CabeceraFacturaCompraService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: CabeceraFacturaCompra): boolean {
      return true;
   }
   ...
   crearEntidad(): CabeceraFacturaCompra {
      const nuevoCabeceraFacturaCompra = new CabeceraFacturaCompra();
      nuevoCabeceraFacturaCompra.id = 0;
      return nuevoCabeceraFacturaCompra;
   }

   add(entidadNueva: CabeceraFacturaCompra): void {
   ...
   update(entidadParaActualizar: CabeceraFacturaCompra): void {
   ...
   delete(entidadParaBorrar: CabeceraFacturaCompra): void {
   ...
   refresh(): void {
      ...
      this.entidades = CabeceraFacturaCompra[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: CabeceraFacturaCompra): void {
