...
@Component({
   selector: 'app-tipouso',
   templateUrl: './tipouso.component.html',
   styleUrls: ['./tipouso.component.scss']
})
...
export class TipoUsoComponent implements OnInit {
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: TipoUsoService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: TipoUso): boolean {
      return true;
   }
   ...
   crearEntidad(): TipoUso {
      const nuevoTipoUso = new TipoUso();
      nuevoTipoUso.id = 0;
      return nuevoTipoUso;
   }

   add(entidadNueva: TipoUso): void {
   ...
   update(entidadParaActualizar: TipoUso): void {
   ...
   delete(entidadParaBorrar: TipoUso): void {
   ...
   onSelect(entidadActual: TipoUso): void {
